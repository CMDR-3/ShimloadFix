#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: P_kerfus_p

#include "Basic.hpp"

#include "P_kerfus_classes.hpp"


namespace SDK
{

// BlueprintGeneratedClass p_kerfus_p.p_kerfus_p_C
// 0x0000 (0x0488 - 0x0488)
class AP_kerfus_p_C final : public AP_kerfus_C
{
public:
	static class UClass* StaticClass()
	{
		return StaticBPGeneratedClassImpl<"p_kerfus_p_C">();
	}
	static class AP_kerfus_p_C* GetDefaultObj()
	{
		return GetDefaultObjImpl<AP_kerfus_p_C>();
	}
};
static_assert(alignof(AP_kerfus_p_C) == 0x000008, "Wrong alignment on AP_kerfus_p_C");
static_assert(sizeof(AP_kerfus_p_C) == 0x000488, "Wrong size on AP_kerfus_p_C");

}

