#pragma once

/*
* SDK generated by Dumper-7
*
* https://github.com/Encryqed/Dumper-7
*/

// Package: Umg_coords

#include "Basic.hpp"

#include "Struct_settings_structs.hpp"
#include "Struct_save_structs.hpp"
#include "SlateCore_structs.hpp"
#include "Struct_signal_spawn_structs.hpp"
#include "CoreUObject_structs.hpp"
#include "UMG_structs.hpp"
#include "InputCore_structs.hpp"
#include "Struct_spaceObject_structs.hpp"


namespace SDK::Params
{

// Function umg_coords.umg_coords_C.ExecuteUbergraph_umg_coords
// 0x0320 (0x0320 - 0x0000)
struct Umg_coords_C_ExecuteUbergraph_umg_coords final
{
public:
	int32                                         EntryPoint;                                        // 0x0000(0x0004)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	TDelegate<void(bool Active_calc, bool Active_downl, bool Active_coords, bool Active_play, bool Active_light)> K2Node_CreateDelegate_OutputDelegate;              // 0x0004(0x0010)(ZeroConstructor, NoDestructor)
	bool                                          Temp_bool_Variable;                                // 0x0014(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	ESlateVisibility                              Temp_byte_Variable;                                // 0x0015(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	ESlateVisibility                              Temp_byte_Variable_1;                              // 0x0016(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	ESlateVisibility                              K2Node_Select_Default;                             // 0x0017(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FKey                                   K2Node_Event_key;                                  // 0x0018(0x0018)(HasGetValueTypeHash)
	bool                                          K2Node_Event_pressed;                              // 0x0030(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_31[0x3];                                       // 0x0031(0x0003)(Fixing Size After Last Property [ Dumper-7 ])
	struct FLinearColor                           K2Node_Event_color;                                // 0x0034(0x0010)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	uint8                                         Pad_44[0x4];                                       // 0x0044(0x0004)(Fixing Size After Last Property [ Dumper-7 ])
	class UUmg_texturePicker_C*                   K2Node_Event_picker;                               // 0x0048(0x0008)(ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	class UTexture2D*                             K2Node_Event_texture;                              // 0x0050(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	int32                                         K2Node_Event_index;                                // 0x0058(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FFocusEvent                            K2Node_Event_InFocusEvent;                         // 0x005C(0x0008)(NoDestructor)
	uint8                                         Pad_64[0x4];                                       // 0x0064(0x0004)(Fixing Size After Last Property [ Dumper-7 ])
	struct FStruct_settings                       K2Node_Event_settings;                             // 0x0068(0x0100)(HasGetValueTypeHash)
	float                                         CallFunc_FClamp_ReturnValue;                       // 0x0168(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	bool                                          CallFunc_NotEqual_NameName_ReturnValue;            // 0x016C(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_16D[0x3];                                      // 0x016D(0x0003)(Fixing Size After Last Property [ Dumper-7 ])
	class UMaterialInstanceDynamic*               CallFunc_GetDynamicMaterial_ReturnValue;           // 0x0170(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_GetWorldDeltaSeconds_ReturnValue;         // 0x0178(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	bool                                          CallFunc_EqualEqual_FloatFloat_ReturnValue;        // 0x017C(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          CallFunc_gatherSignal_return;                      // 0x017D(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_17E[0x2];                                      // 0x017E(0x0002)(Fixing Size After Last Property [ Dumper-7 ])
	int32                                         CallFunc_gatherSignal_index;                       // 0x0180(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FStruct_signal_spawn                   CallFunc_gatherSignal_data;                        // 0x0184(0x002C)(IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FStruct_spaceObject                    CallFunc_GetDataTableRowFromName_OutRow;           // 0x01B0(0x0068)(HasGetValueTypeHash)
	bool                                          CallFunc_GetDataTableRowFromName_ReturnValue;      // 0x0218(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_219[0x3];                                      // 0x0219(0x0003)(Fixing Size After Last Property [ Dumper-7 ])
	TDelegate<void()>                             K2Node_CreateDelegate_OutputDelegate_1;            // 0x021C(0x0010)(ZeroConstructor, NoDestructor)
	struct FVector2D                              CallFunc_Conv_VectorToVector2D_ReturnValue;        // 0x022C(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	bool                                          K2Node_CustomEvent_active_calc;                    // 0x0234(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          K2Node_CustomEvent_active_downl;                   // 0x0235(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          K2Node_CustomEvent_active_coords;                  // 0x0236(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          K2Node_CustomEvent_active_play;                    // 0x0237(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          K2Node_CustomEvent_active_light;                   // 0x0238(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_239[0x7];                                      // 0x0239(0x0007)(Fixing Size After Last Property [ Dumper-7 ])
	class AMainGamemode_C*                        K2Node_Event_gamemode;                             // 0x0240(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FVector2D                              CallFunc_normCoords_ReturnValue;                   // 0x0248(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FVector                                CallFunc_getCoords_return;                         // 0x0250(0x000C)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FVector                                CallFunc_Multiply_VectorFloat_ReturnValue;         // 0x025C(0x000C)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FVector                                CallFunc_Add_VectorVector_ReturnValue;             // 0x0268(0x000C)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	bool                                          CallFunc_LessEqual_FloatFloat_ReturnValue;         // 0x0274(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_275[0x3];                                      // 0x0275(0x0003)(Fixing Size After Last Property [ Dumper-7 ])
	float                                         CallFunc_GetWorldDeltaSeconds_ReturnValue_1;       // 0x0278(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_VictoryFloatMinusEquals_FloatOut;         // 0x027C(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_FMax_ReturnValue;                         // 0x0280(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	uint8                                         Pad_284[0x4];                                      // 0x0284(0x0004)(Fixing Size After Last Property [ Dumper-7 ])
	class FText                                   CallFunc_Conv_FloatToText_ReturnValue;             // 0x0288(0x0018)()
	int32                                         Temp_int_Variable;                                 // 0x02A0(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	int32                                         CallFunc_Add_IntInt_ReturnValue;                   // 0x02A4(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	TScriptInterface<class IInt_ttrigger_C>       K2Node_DynamicCast_AsInt_Ttrigger;                 // 0x02A8(0x0010)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          K2Node_DynamicCast_bSuccess;                       // 0x02B8(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          CallFunc_IsValid_ReturnValue;                      // 0x02B9(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_2BA[0x6];                                      // 0x02BA(0x0006)(Fixing Size After Last Property [ Dumper-7 ])
	class ADish_C*                                CallFunc_Array_Get_Item;                           // 0x02C0(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	int32                                         CallFunc_Array_Length_ReturnValue;                 // 0x02C8(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	bool                                          CallFunc_Less_IntInt_ReturnValue;                  // 0x02CC(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_2CD[0x3];                                      // 0x02CD(0x0003)(Fixing Size After Last Property [ Dumper-7 ])
	int32                                         CallFunc_Subtract_IntInt_ReturnValue;              // 0x02D0(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	int32                                         CallFunc_Subtract_IntInt_ReturnValue_1;            // 0x02D4(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Conv_IntToFloat_ReturnValue;              // 0x02D8(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Conv_IntToFloat_ReturnValue_1;            // 0x02DC(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Divide_FloatFloat_ReturnValue;            // 0x02E0(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Divide_FloatFloat_ReturnValue_1;          // 0x02E4(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Lerp_ReturnValue;                         // 0x02E8(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Lerp_ReturnValue_1;                       // 0x02EC(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Multiply_FloatFloat_ReturnValue;          // 0x02F0(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_VictoryFloatPlusEquals_FloatOut;          // 0x02F4(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	class AMainGamemode_C*                        CallFunc_getMainGamemode_AsMain_Gamemode;          // 0x02F8(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	bool                                          CallFunc_IsValid_ReturnValue_1;                    // 0x0300(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_301[0x3];                                      // 0x0301(0x0003)(Fixing Size After Last Property [ Dumper-7 ])
	int32                                         CallFunc_Subtract_IntInt_ReturnValue_2;            // 0x0304(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Conv_IntToFloat_ReturnValue_2;            // 0x0308(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Divide_FloatFloat_ReturnValue_2;          // 0x030C(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_MultiplyMultiply_FloatFloat_ReturnValue;  // 0x0310(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Lerp_ReturnValue_2;                       // 0x0314(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	bool                                          CallFunc_RandomBoolWithWeight_ReturnValue;         // 0x0318(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
};
static_assert(alignof(Umg_coords_C_ExecuteUbergraph_umg_coords) == 0x000008, "Wrong alignment on Umg_coords_C_ExecuteUbergraph_umg_coords");
static_assert(sizeof(Umg_coords_C_ExecuteUbergraph_umg_coords) == 0x000320, "Wrong size on Umg_coords_C_ExecuteUbergraph_umg_coords");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, EntryPoint) == 0x000000, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::EntryPoint' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_CreateDelegate_OutputDelegate) == 0x000004, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_CreateDelegate_OutputDelegate' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, Temp_bool_Variable) == 0x000014, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::Temp_bool_Variable' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, Temp_byte_Variable) == 0x000015, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::Temp_byte_Variable' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, Temp_byte_Variable_1) == 0x000016, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::Temp_byte_Variable_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_Select_Default) == 0x000017, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_Select_Default' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_Event_key) == 0x000018, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_Event_key' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_Event_pressed) == 0x000030, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_Event_pressed' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_Event_color) == 0x000034, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_Event_color' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_Event_picker) == 0x000048, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_Event_picker' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_Event_texture) == 0x000050, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_Event_texture' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_Event_index) == 0x000058, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_Event_index' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_Event_InFocusEvent) == 0x00005C, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_Event_InFocusEvent' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_Event_settings) == 0x000068, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_Event_settings' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_FClamp_ReturnValue) == 0x000168, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_FClamp_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_NotEqual_NameName_ReturnValue) == 0x00016C, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_NotEqual_NameName_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_GetDynamicMaterial_ReturnValue) == 0x000170, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_GetDynamicMaterial_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_GetWorldDeltaSeconds_ReturnValue) == 0x000178, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_GetWorldDeltaSeconds_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_EqualEqual_FloatFloat_ReturnValue) == 0x00017C, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_EqualEqual_FloatFloat_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_gatherSignal_return) == 0x00017D, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_gatherSignal_return' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_gatherSignal_index) == 0x000180, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_gatherSignal_index' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_gatherSignal_data) == 0x000184, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_gatherSignal_data' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_GetDataTableRowFromName_OutRow) == 0x0001B0, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_GetDataTableRowFromName_OutRow' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_GetDataTableRowFromName_ReturnValue) == 0x000218, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_GetDataTableRowFromName_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_CreateDelegate_OutputDelegate_1) == 0x00021C, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_CreateDelegate_OutputDelegate_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Conv_VectorToVector2D_ReturnValue) == 0x00022C, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Conv_VectorToVector2D_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_CustomEvent_active_calc) == 0x000234, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_CustomEvent_active_calc' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_CustomEvent_active_downl) == 0x000235, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_CustomEvent_active_downl' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_CustomEvent_active_coords) == 0x000236, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_CustomEvent_active_coords' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_CustomEvent_active_play) == 0x000237, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_CustomEvent_active_play' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_CustomEvent_active_light) == 0x000238, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_CustomEvent_active_light' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_Event_gamemode) == 0x000240, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_Event_gamemode' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_normCoords_ReturnValue) == 0x000248, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_normCoords_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_getCoords_return) == 0x000250, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_getCoords_return' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Multiply_VectorFloat_ReturnValue) == 0x00025C, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Multiply_VectorFloat_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Add_VectorVector_ReturnValue) == 0x000268, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Add_VectorVector_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_LessEqual_FloatFloat_ReturnValue) == 0x000274, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_LessEqual_FloatFloat_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_GetWorldDeltaSeconds_ReturnValue_1) == 0x000278, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_GetWorldDeltaSeconds_ReturnValue_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_VictoryFloatMinusEquals_FloatOut) == 0x00027C, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_VictoryFloatMinusEquals_FloatOut' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_FMax_ReturnValue) == 0x000280, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_FMax_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Conv_FloatToText_ReturnValue) == 0x000288, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Conv_FloatToText_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, Temp_int_Variable) == 0x0002A0, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::Temp_int_Variable' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Add_IntInt_ReturnValue) == 0x0002A4, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Add_IntInt_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_DynamicCast_AsInt_Ttrigger) == 0x0002A8, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_DynamicCast_AsInt_Ttrigger' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, K2Node_DynamicCast_bSuccess) == 0x0002B8, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::K2Node_DynamicCast_bSuccess' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_IsValid_ReturnValue) == 0x0002B9, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_IsValid_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Array_Get_Item) == 0x0002C0, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Array_Get_Item' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Array_Length_ReturnValue) == 0x0002C8, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Array_Length_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Less_IntInt_ReturnValue) == 0x0002CC, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Less_IntInt_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Subtract_IntInt_ReturnValue) == 0x0002D0, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Subtract_IntInt_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Subtract_IntInt_ReturnValue_1) == 0x0002D4, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Subtract_IntInt_ReturnValue_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Conv_IntToFloat_ReturnValue) == 0x0002D8, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Conv_IntToFloat_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Conv_IntToFloat_ReturnValue_1) == 0x0002DC, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Conv_IntToFloat_ReturnValue_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Divide_FloatFloat_ReturnValue) == 0x0002E0, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Divide_FloatFloat_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Divide_FloatFloat_ReturnValue_1) == 0x0002E4, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Divide_FloatFloat_ReturnValue_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Lerp_ReturnValue) == 0x0002E8, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Lerp_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Lerp_ReturnValue_1) == 0x0002EC, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Lerp_ReturnValue_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Multiply_FloatFloat_ReturnValue) == 0x0002F0, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Multiply_FloatFloat_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_VictoryFloatPlusEquals_FloatOut) == 0x0002F4, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_VictoryFloatPlusEquals_FloatOut' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_getMainGamemode_AsMain_Gamemode) == 0x0002F8, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_getMainGamemode_AsMain_Gamemode' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_IsValid_ReturnValue_1) == 0x000300, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_IsValid_ReturnValue_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Subtract_IntInt_ReturnValue_2) == 0x000304, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Subtract_IntInt_ReturnValue_2' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Conv_IntToFloat_ReturnValue_2) == 0x000308, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Conv_IntToFloat_ReturnValue_2' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Divide_FloatFloat_ReturnValue_2) == 0x00030C, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Divide_FloatFloat_ReturnValue_2' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_MultiplyMultiply_FloatFloat_ReturnValue) == 0x000310, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_MultiplyMultiply_FloatFloat_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_Lerp_ReturnValue_2) == 0x000314, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_Lerp_ReturnValue_2' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_ExecuteUbergraph_umg_coords, CallFunc_RandomBoolWithWeight_ReturnValue) == 0x000318, "Member 'Umg_coords_C_ExecuteUbergraph_umg_coords::CallFunc_RandomBoolWithWeight_ReturnValue' has a wrong offset!");

// Function umg_coords.umg_coords_C.stuffUpgraded
// 0x0008 (0x0008 - 0x0000)
struct Umg_coords_C_StuffUpgraded final
{
public:
	class AMainGamemode_C*                        GameMode;                                          // 0x0000(0x0008)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
};
static_assert(alignof(Umg_coords_C_StuffUpgraded) == 0x000008, "Wrong alignment on Umg_coords_C_StuffUpgraded");
static_assert(sizeof(Umg_coords_C_StuffUpgraded) == 0x000008, "Wrong size on Umg_coords_C_StuffUpgraded");
static_assert(offsetof(Umg_coords_C_StuffUpgraded, GameMode) == 0x000000, "Member 'Umg_coords_C_StuffUpgraded::GameMode' has a wrong offset!");

// Function umg_coords.umg_coords_C.powerChanged
// 0x0005 (0x0005 - 0x0000)
struct Umg_coords_C_PowerChanged final
{
public:
	bool                                          Active_calc;                                       // 0x0000(0x0001)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          Active_downl;                                      // 0x0001(0x0001)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          Active_coords;                                     // 0x0002(0x0001)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          Active_play;                                       // 0x0003(0x0001)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          Active_light;                                      // 0x0004(0x0001)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)
};
static_assert(alignof(Umg_coords_C_PowerChanged) == 0x000001, "Wrong alignment on Umg_coords_C_PowerChanged");
static_assert(sizeof(Umg_coords_C_PowerChanged) == 0x000005, "Wrong size on Umg_coords_C_PowerChanged");
static_assert(offsetof(Umg_coords_C_PowerChanged, Active_calc) == 0x000000, "Member 'Umg_coords_C_PowerChanged::Active_calc' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_PowerChanged, Active_downl) == 0x000001, "Member 'Umg_coords_C_PowerChanged::Active_downl' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_PowerChanged, Active_coords) == 0x000002, "Member 'Umg_coords_C_PowerChanged::Active_coords' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_PowerChanged, Active_play) == 0x000003, "Member 'Umg_coords_C_PowerChanged::Active_play' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_PowerChanged, Active_light) == 0x000004, "Member 'Umg_coords_C_PowerChanged::Active_light' has a wrong offset!");

// Function umg_coords.umg_coords_C.settingsApplied
// 0x0100 (0x0100 - 0x0000)
struct Umg_coords_C_SettingsApplied final
{
public:
	struct FStruct_settings                       Settings;                                          // 0x0000(0x0100)(BlueprintVisible, BlueprintReadOnly, Parm, HasGetValueTypeHash)
};
static_assert(alignof(Umg_coords_C_SettingsApplied) == 0x000008, "Wrong alignment on Umg_coords_C_SettingsApplied");
static_assert(sizeof(Umg_coords_C_SettingsApplied) == 0x000100, "Wrong size on Umg_coords_C_SettingsApplied");
static_assert(offsetof(Umg_coords_C_SettingsApplied, Settings) == 0x000000, "Member 'Umg_coords_C_SettingsApplied::Settings' has a wrong offset!");

// Function umg_coords.umg_coords_C.OnFocusLost
// 0x0008 (0x0008 - 0x0000)
struct Umg_coords_C_OnFocusLost final
{
public:
	struct FFocusEvent                            InFocusEvent;                                      // 0x0000(0x0008)(BlueprintVisible, BlueprintReadOnly, Parm, NoDestructor)
};
static_assert(alignof(Umg_coords_C_OnFocusLost) == 0x000004, "Wrong alignment on Umg_coords_C_OnFocusLost");
static_assert(sizeof(Umg_coords_C_OnFocusLost) == 0x000008, "Wrong size on Umg_coords_C_OnFocusLost");
static_assert(offsetof(Umg_coords_C_OnFocusLost, InFocusEvent) == 0x000000, "Member 'Umg_coords_C_OnFocusLost::InFocusEvent' has a wrong offset!");

// Function umg_coords.umg_coords_C.texturePickerApply
// 0x0018 (0x0018 - 0x0000)
struct Umg_coords_C_TexturePickerApply final
{
public:
	class UUmg_texturePicker_C*                   Picker;                                            // 0x0000(0x0008)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	class UTexture2D*                             Texture;                                           // 0x0008(0x0008)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	int32                                         Index_0;                                           // 0x0010(0x0004)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
};
static_assert(alignof(Umg_coords_C_TexturePickerApply) == 0x000008, "Wrong alignment on Umg_coords_C_TexturePickerApply");
static_assert(sizeof(Umg_coords_C_TexturePickerApply) == 0x000018, "Wrong size on Umg_coords_C_TexturePickerApply");
static_assert(offsetof(Umg_coords_C_TexturePickerApply, Picker) == 0x000000, "Member 'Umg_coords_C_TexturePickerApply::Picker' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_TexturePickerApply, Texture) == 0x000008, "Member 'Umg_coords_C_TexturePickerApply::Texture' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_TexturePickerApply, Index_0) == 0x000010, "Member 'Umg_coords_C_TexturePickerApply::Index_0' has a wrong offset!");

// Function umg_coords.umg_coords_C.applyColor
// 0x0010 (0x0010 - 0x0000)
struct Umg_coords_C_ApplyColor final
{
public:
	struct FLinearColor                           Color;                                             // 0x0000(0x0010)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
};
static_assert(alignof(Umg_coords_C_ApplyColor) == 0x000004, "Wrong alignment on Umg_coords_C_ApplyColor");
static_assert(sizeof(Umg_coords_C_ApplyColor) == 0x000010, "Wrong size on Umg_coords_C_ApplyColor");
static_assert(offsetof(Umg_coords_C_ApplyColor, Color) == 0x000000, "Member 'Umg_coords_C_ApplyColor::Color' has a wrong offset!");

// Function umg_coords.umg_coords_C.AnyKey
// 0x0020 (0x0020 - 0x0000)
struct Umg_coords_C_AnyKey final
{
public:
	struct FKey                                   Key;                                               // 0x0000(0x0018)(BlueprintVisible, BlueprintReadOnly, Parm, HasGetValueTypeHash)
	bool                                          Pressed;                                           // 0x0018(0x0001)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor)
};
static_assert(alignof(Umg_coords_C_AnyKey) == 0x000008, "Wrong alignment on Umg_coords_C_AnyKey");
static_assert(sizeof(Umg_coords_C_AnyKey) == 0x000020, "Wrong size on Umg_coords_C_AnyKey");
static_assert(offsetof(Umg_coords_C_AnyKey, Key) == 0x000000, "Member 'Umg_coords_C_AnyKey::Key' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_AnyKey, Pressed) == 0x000018, "Member 'Umg_coords_C_AnyKey::Pressed' has a wrong offset!");

// Function umg_coords.umg_coords_C.OnKeyDown
// 0x02D0 (0x02D0 - 0x0000)
struct Umg_coords_C_OnKeyDown final
{
public:
	struct FGeometry                              MyGeometry;                                        // 0x0000(0x0038)(BlueprintVisible, BlueprintReadOnly, Parm, IsPlainOldData, NoDestructor)
	struct FKeyEvent                              InKeyEvent;                                        // 0x0038(0x0038)(BlueprintVisible, BlueprintReadOnly, Parm)
	struct FEventReply                            ReturnValue;                                       // 0x0070(0x00B8)(Parm, OutParm, ReturnParm)
	struct FKey                                   Key;                                               // 0x0128(0x0018)(Edit, BlueprintVisible, HasGetValueTypeHash)
	bool                                          Temp_bool_Variable;                                // 0x0140(0x0001)(ConstParm, ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_141[0x7];                                      // 0x0141(0x0007)(Fixing Size After Last Property [ Dumper-7 ])
	struct FKey                                   CallFunc_GetKey_ReturnValue;                       // 0x0148(0x0018)(HasGetValueTypeHash)
	struct FKey                                   CallFunc_getKeybindFromName_bind;                  // 0x0160(0x0018)(HasGetValueTypeHash)
	struct FKey                                   CallFunc_getKeybindFromName_bind_1;                // 0x0178(0x0018)(HasGetValueTypeHash)
	struct FKey                                   CallFunc_getKeybindFromName_bind_2;                // 0x0190(0x0018)(HasGetValueTypeHash)
	struct FKey                                   CallFunc_getKeybindFromName_bind_3;                // 0x01A8(0x0018)(HasGetValueTypeHash)
	struct FKey                                   CallFunc_getKeybindFromName_bind_4;                // 0x01C0(0x0018)(HasGetValueTypeHash)
	bool                                          CallFunc_IsValid_ReturnValue;                      // 0x01D8(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          CallFunc_Array_Contains_ReturnValue;               // 0x01D9(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_1DA[0x2];                                      // 0x01DA(0x0002)(Fixing Size After Last Property [ Dumper-7 ])
	int32                                         CallFunc_Subtract_IntInt_ReturnValue;              // 0x01DC(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Conv_IntToFloat_ReturnValue;              // 0x01E0(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Divide_FloatFloat_ReturnValue;            // 0x01E4(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	bool                                          CallFunc_EqualEqual_KeyKey_ReturnValue;            // 0x01E8(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_1E9[0x3];                                      // 0x01E9(0x0003)(Fixing Size After Last Property [ Dumper-7 ])
	float                                         CallFunc_Lerp_ReturnValue;                         // 0x01EC(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FVector2D                              CallFunc_MakeVector2D_ReturnValue;                 // 0x01F0(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FKey                                   CallFunc_getKeybindFromName_bind_5;                // 0x01F8(0x0018)(HasGetValueTypeHash)
	bool                                          CallFunc_EqualEqual_KeyKey_ReturnValue_1;          // 0x0210(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          CallFunc_EqualEqual_KeyKey_ReturnValue_2;          // 0x0211(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          CallFunc_EqualEqual_KeyKey_ReturnValue_3;          // 0x0212(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          CallFunc_EqualEqual_KeyKey_ReturnValue_4;          // 0x0213(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          CallFunc_EqualEqual_KeyKey_ReturnValue_5;          // 0x0214(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_215[0x3];                                      // 0x0215(0x0003)(Fixing Size After Last Property [ Dumper-7 ])
	struct FEventReply                            K2Node_MakeStruct_EventReply;                      // 0x0218(0x00B8)()
};
static_assert(alignof(Umg_coords_C_OnKeyDown) == 0x000008, "Wrong alignment on Umg_coords_C_OnKeyDown");
static_assert(sizeof(Umg_coords_C_OnKeyDown) == 0x0002D0, "Wrong size on Umg_coords_C_OnKeyDown");
static_assert(offsetof(Umg_coords_C_OnKeyDown, MyGeometry) == 0x000000, "Member 'Umg_coords_C_OnKeyDown::MyGeometry' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, InKeyEvent) == 0x000038, "Member 'Umg_coords_C_OnKeyDown::InKeyEvent' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, ReturnValue) == 0x000070, "Member 'Umg_coords_C_OnKeyDown::ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, Key) == 0x000128, "Member 'Umg_coords_C_OnKeyDown::Key' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, Temp_bool_Variable) == 0x000140, "Member 'Umg_coords_C_OnKeyDown::Temp_bool_Variable' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_GetKey_ReturnValue) == 0x000148, "Member 'Umg_coords_C_OnKeyDown::CallFunc_GetKey_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_getKeybindFromName_bind) == 0x000160, "Member 'Umg_coords_C_OnKeyDown::CallFunc_getKeybindFromName_bind' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_getKeybindFromName_bind_1) == 0x000178, "Member 'Umg_coords_C_OnKeyDown::CallFunc_getKeybindFromName_bind_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_getKeybindFromName_bind_2) == 0x000190, "Member 'Umg_coords_C_OnKeyDown::CallFunc_getKeybindFromName_bind_2' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_getKeybindFromName_bind_3) == 0x0001A8, "Member 'Umg_coords_C_OnKeyDown::CallFunc_getKeybindFromName_bind_3' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_getKeybindFromName_bind_4) == 0x0001C0, "Member 'Umg_coords_C_OnKeyDown::CallFunc_getKeybindFromName_bind_4' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_IsValid_ReturnValue) == 0x0001D8, "Member 'Umg_coords_C_OnKeyDown::CallFunc_IsValid_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_Array_Contains_ReturnValue) == 0x0001D9, "Member 'Umg_coords_C_OnKeyDown::CallFunc_Array_Contains_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_Subtract_IntInt_ReturnValue) == 0x0001DC, "Member 'Umg_coords_C_OnKeyDown::CallFunc_Subtract_IntInt_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_Conv_IntToFloat_ReturnValue) == 0x0001E0, "Member 'Umg_coords_C_OnKeyDown::CallFunc_Conv_IntToFloat_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_Divide_FloatFloat_ReturnValue) == 0x0001E4, "Member 'Umg_coords_C_OnKeyDown::CallFunc_Divide_FloatFloat_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_EqualEqual_KeyKey_ReturnValue) == 0x0001E8, "Member 'Umg_coords_C_OnKeyDown::CallFunc_EqualEqual_KeyKey_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_Lerp_ReturnValue) == 0x0001EC, "Member 'Umg_coords_C_OnKeyDown::CallFunc_Lerp_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_MakeVector2D_ReturnValue) == 0x0001F0, "Member 'Umg_coords_C_OnKeyDown::CallFunc_MakeVector2D_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_getKeybindFromName_bind_5) == 0x0001F8, "Member 'Umg_coords_C_OnKeyDown::CallFunc_getKeybindFromName_bind_5' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_EqualEqual_KeyKey_ReturnValue_1) == 0x000210, "Member 'Umg_coords_C_OnKeyDown::CallFunc_EqualEqual_KeyKey_ReturnValue_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_EqualEqual_KeyKey_ReturnValue_2) == 0x000211, "Member 'Umg_coords_C_OnKeyDown::CallFunc_EqualEqual_KeyKey_ReturnValue_2' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_EqualEqual_KeyKey_ReturnValue_3) == 0x000212, "Member 'Umg_coords_C_OnKeyDown::CallFunc_EqualEqual_KeyKey_ReturnValue_3' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_EqualEqual_KeyKey_ReturnValue_4) == 0x000213, "Member 'Umg_coords_C_OnKeyDown::CallFunc_EqualEqual_KeyKey_ReturnValue_4' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, CallFunc_EqualEqual_KeyKey_ReturnValue_5) == 0x000214, "Member 'Umg_coords_C_OnKeyDown::CallFunc_EqualEqual_KeyKey_ReturnValue_5' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyDown, K2Node_MakeStruct_EventReply) == 0x000218, "Member 'Umg_coords_C_OnKeyDown::K2Node_MakeStruct_EventReply' has a wrong offset!");

// Function umg_coords.umg_coords_C.OnKeyUp
// 0x0278 (0x0278 - 0x0000)
struct Umg_coords_C_OnKeyUp final
{
public:
	struct FGeometry                              MyGeometry;                                        // 0x0000(0x0038)(BlueprintVisible, BlueprintReadOnly, Parm, IsPlainOldData, NoDestructor)
	struct FKeyEvent                              InKeyEvent;                                        // 0x0038(0x0038)(BlueprintVisible, BlueprintReadOnly, Parm)
	struct FEventReply                            ReturnValue;                                       // 0x0070(0x00B8)(Parm, OutParm, ReturnParm)
	struct FKey                                   Key;                                               // 0x0128(0x0018)(Edit, BlueprintVisible, HasGetValueTypeHash)
	struct FKey                                   CallFunc_GetKey_ReturnValue;                       // 0x0140(0x0018)(HasGetValueTypeHash)
	struct FKey                                   CallFunc_getKeybindFromName_bind;                  // 0x0158(0x0018)(HasGetValueTypeHash)
	struct FKey                                   CallFunc_getKeybindFromName_bind_1;                // 0x0170(0x0018)(HasGetValueTypeHash)
	struct FKey                                   CallFunc_getKeybindFromName_bind_2;                // 0x0188(0x0018)(HasGetValueTypeHash)
	struct FKey                                   CallFunc_getKeybindFromName_bind_3;                // 0x01A0(0x0018)(HasGetValueTypeHash)
	bool                                          CallFunc_EqualEqual_KeyKey_ReturnValue;            // 0x01B8(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          CallFunc_EqualEqual_KeyKey_ReturnValue_1;          // 0x01B9(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          CallFunc_EqualEqual_KeyKey_ReturnValue_2;          // 0x01BA(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	bool                                          CallFunc_EqualEqual_KeyKey_ReturnValue_3;          // 0x01BB(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
	uint8                                         Pad_1BC[0x4];                                      // 0x01BC(0x0004)(Fixing Size After Last Property [ Dumper-7 ])
	struct FEventReply                            K2Node_MakeStruct_EventReply;                      // 0x01C0(0x00B8)()
};
static_assert(alignof(Umg_coords_C_OnKeyUp) == 0x000008, "Wrong alignment on Umg_coords_C_OnKeyUp");
static_assert(sizeof(Umg_coords_C_OnKeyUp) == 0x000278, "Wrong size on Umg_coords_C_OnKeyUp");
static_assert(offsetof(Umg_coords_C_OnKeyUp, MyGeometry) == 0x000000, "Member 'Umg_coords_C_OnKeyUp::MyGeometry' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, InKeyEvent) == 0x000038, "Member 'Umg_coords_C_OnKeyUp::InKeyEvent' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, ReturnValue) == 0x000070, "Member 'Umg_coords_C_OnKeyUp::ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, Key) == 0x000128, "Member 'Umg_coords_C_OnKeyUp::Key' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, CallFunc_GetKey_ReturnValue) == 0x000140, "Member 'Umg_coords_C_OnKeyUp::CallFunc_GetKey_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, CallFunc_getKeybindFromName_bind) == 0x000158, "Member 'Umg_coords_C_OnKeyUp::CallFunc_getKeybindFromName_bind' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, CallFunc_getKeybindFromName_bind_1) == 0x000170, "Member 'Umg_coords_C_OnKeyUp::CallFunc_getKeybindFromName_bind_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, CallFunc_getKeybindFromName_bind_2) == 0x000188, "Member 'Umg_coords_C_OnKeyUp::CallFunc_getKeybindFromName_bind_2' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, CallFunc_getKeybindFromName_bind_3) == 0x0001A0, "Member 'Umg_coords_C_OnKeyUp::CallFunc_getKeybindFromName_bind_3' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, CallFunc_EqualEqual_KeyKey_ReturnValue) == 0x0001B8, "Member 'Umg_coords_C_OnKeyUp::CallFunc_EqualEqual_KeyKey_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, CallFunc_EqualEqual_KeyKey_ReturnValue_1) == 0x0001B9, "Member 'Umg_coords_C_OnKeyUp::CallFunc_EqualEqual_KeyKey_ReturnValue_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, CallFunc_EqualEqual_KeyKey_ReturnValue_2) == 0x0001BA, "Member 'Umg_coords_C_OnKeyUp::CallFunc_EqualEqual_KeyKey_ReturnValue_2' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, CallFunc_EqualEqual_KeyKey_ReturnValue_3) == 0x0001BB, "Member 'Umg_coords_C_OnKeyUp::CallFunc_EqualEqual_KeyKey_ReturnValue_3' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_OnKeyUp, K2Node_MakeStruct_EventReply) == 0x0001C0, "Member 'Umg_coords_C_OnKeyUp::K2Node_MakeStruct_EventReply' has a wrong offset!");

// Function umg_coords.umg_coords_C.updateText
// 0x0040 (0x0040 - 0x0000)
struct Umg_coords_C_UpdateText final
{
public:
	struct FVector2D                              CallFunc_normCoords_ReturnValue;                   // 0x0000(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_BreakVector2D_X;                          // 0x0008(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_BreakVector2D_Y;                          // 0x000C(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	class FText                                   CallFunc_Conv_FloatToText_ReturnValue;             // 0x0010(0x0018)()
	class FText                                   CallFunc_Conv_FloatToText_ReturnValue_1;           // 0x0028(0x0018)()
};
static_assert(alignof(Umg_coords_C_UpdateText) == 0x000008, "Wrong alignment on Umg_coords_C_UpdateText");
static_assert(sizeof(Umg_coords_C_UpdateText) == 0x000040, "Wrong size on Umg_coords_C_UpdateText");
static_assert(offsetof(Umg_coords_C_UpdateText, CallFunc_normCoords_ReturnValue) == 0x000000, "Member 'Umg_coords_C_UpdateText::CallFunc_normCoords_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_UpdateText, CallFunc_BreakVector2D_X) == 0x000008, "Member 'Umg_coords_C_UpdateText::CallFunc_BreakVector2D_X' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_UpdateText, CallFunc_BreakVector2D_Y) == 0x00000C, "Member 'Umg_coords_C_UpdateText::CallFunc_BreakVector2D_Y' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_UpdateText, CallFunc_Conv_FloatToText_ReturnValue) == 0x000010, "Member 'Umg_coords_C_UpdateText::CallFunc_Conv_FloatToText_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_UpdateText, CallFunc_Conv_FloatToText_ReturnValue_1) == 0x000028, "Member 'Umg_coords_C_UpdateText::CallFunc_Conv_FloatToText_ReturnValue_1' has a wrong offset!");

// Function umg_coords.umg_coords_C.writeToLog
// 0x0080 (0x0080 - 0x0000)
struct Umg_coords_C_WriteToLog final
{
public:
	class FString                                 Text;                                              // 0x0000(0x0010)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, HasGetValueTypeHash)
	class ADish_C*                                N;                                                 // 0x0010(0x0008)(Edit, BlueprintVisible, ZeroConstructor, DisableEditOnTemplate, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	TArray<class ADish_C*>                        A;                                                 // 0x0018(0x0010)(Edit, BlueprintVisible, DisableEditOnTemplate)
	class FString                                 CallFunc_Right_ReturnValue;                        // 0x0028(0x0010)(ZeroConstructor, HasGetValueTypeHash)
	class FString                                 CallFunc_LeftChop_ReturnValue;                     // 0x0038(0x0010)(ZeroConstructor, HasGetValueTypeHash)
	class FText                                   CallFunc_Conv_StringToText_ReturnValue;            // 0x0048(0x0018)()
	class FString                                 CallFunc_Concat_StrStr_ReturnValue;                // 0x0060(0x0010)(ZeroConstructor, HasGetValueTypeHash)
	class FString                                 CallFunc_Concat_StrStr_ReturnValue_1;              // 0x0070(0x0010)(ZeroConstructor, HasGetValueTypeHash)
};
static_assert(alignof(Umg_coords_C_WriteToLog) == 0x000008, "Wrong alignment on Umg_coords_C_WriteToLog");
static_assert(sizeof(Umg_coords_C_WriteToLog) == 0x000080, "Wrong size on Umg_coords_C_WriteToLog");
static_assert(offsetof(Umg_coords_C_WriteToLog, Text) == 0x000000, "Member 'Umg_coords_C_WriteToLog::Text' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_WriteToLog, N) == 0x000010, "Member 'Umg_coords_C_WriteToLog::N' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_WriteToLog, A) == 0x000018, "Member 'Umg_coords_C_WriteToLog::A' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_WriteToLog, CallFunc_Right_ReturnValue) == 0x000028, "Member 'Umg_coords_C_WriteToLog::CallFunc_Right_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_WriteToLog, CallFunc_LeftChop_ReturnValue) == 0x000038, "Member 'Umg_coords_C_WriteToLog::CallFunc_LeftChop_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_WriteToLog, CallFunc_Conv_StringToText_ReturnValue) == 0x000048, "Member 'Umg_coords_C_WriteToLog::CallFunc_Conv_StringToText_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_WriteToLog, CallFunc_Concat_StrStr_ReturnValue) == 0x000060, "Member 'Umg_coords_C_WriteToLog::CallFunc_Concat_StrStr_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_WriteToLog, CallFunc_Concat_StrStr_ReturnValue_1) == 0x000070, "Member 'Umg_coords_C_WriteToLog::CallFunc_Concat_StrStr_ReturnValue_1' has a wrong offset!");

// Function umg_coords.umg_coords_C.setLoc
// 0x0010 (0x0010 - 0x0000)
struct Umg_coords_C_SetLoc final
{
public:
	struct FVector2D                              InPosition;                                        // 0x0000(0x0008)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	class UCanvasPanelSlot*                       CallFunc_SlotAsCanvasSlot_ReturnValue;             // 0x0008(0x0008)(ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
};
static_assert(alignof(Umg_coords_C_SetLoc) == 0x000008, "Wrong alignment on Umg_coords_C_SetLoc");
static_assert(sizeof(Umg_coords_C_SetLoc) == 0x000010, "Wrong size on Umg_coords_C_SetLoc");
static_assert(offsetof(Umg_coords_C_SetLoc, InPosition) == 0x000000, "Member 'Umg_coords_C_SetLoc::InPosition' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_SetLoc, CallFunc_SlotAsCanvasSlot_ReturnValue) == 0x000008, "Member 'Umg_coords_C_SetLoc::CallFunc_SlotAsCanvasSlot_ReturnValue' has a wrong offset!");

// Function umg_coords.umg_coords_C.normCoords
// 0x0040 (0x0040 - 0x0000)
struct Umg_coords_C_NormCoords final
{
public:
	struct FVector2D                              ReturnValue;                                       // 0x0000(0x0008)(Parm, OutParm, ZeroConstructor, ReturnParm, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FVector2D                              CallFunc_GetAbs2D_ReturnValue;                     // 0x0008(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FVector2D                              CallFunc_Subtract_Vector2DVector2D_ReturnValue;    // 0x0010(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FVector2D                              CallFunc_Divide_Vector2DVector2D_ReturnValue;      // 0x0018(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FVector2D                              CallFunc_Subtract_Vector2DVector2D_ReturnValue_1;  // 0x0020(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_BreakVector2D_X;                          // 0x0028(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_BreakVector2D_Y;                          // 0x002C(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Lerp_ReturnValue;                         // 0x0030(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	float                                         CallFunc_Lerp_ReturnValue_1;                       // 0x0034(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FVector2D                              CallFunc_MakeVector2D_ReturnValue;                 // 0x0038(0x0008)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
};
static_assert(alignof(Umg_coords_C_NormCoords) == 0x000004, "Wrong alignment on Umg_coords_C_NormCoords");
static_assert(sizeof(Umg_coords_C_NormCoords) == 0x000040, "Wrong size on Umg_coords_C_NormCoords");
static_assert(offsetof(Umg_coords_C_NormCoords, ReturnValue) == 0x000000, "Member 'Umg_coords_C_NormCoords::ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_NormCoords, CallFunc_GetAbs2D_ReturnValue) == 0x000008, "Member 'Umg_coords_C_NormCoords::CallFunc_GetAbs2D_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_NormCoords, CallFunc_Subtract_Vector2DVector2D_ReturnValue) == 0x000010, "Member 'Umg_coords_C_NormCoords::CallFunc_Subtract_Vector2DVector2D_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_NormCoords, CallFunc_Divide_Vector2DVector2D_ReturnValue) == 0x000018, "Member 'Umg_coords_C_NormCoords::CallFunc_Divide_Vector2DVector2D_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_NormCoords, CallFunc_Subtract_Vector2DVector2D_ReturnValue_1) == 0x000020, "Member 'Umg_coords_C_NormCoords::CallFunc_Subtract_Vector2DVector2D_ReturnValue_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_NormCoords, CallFunc_BreakVector2D_X) == 0x000028, "Member 'Umg_coords_C_NormCoords::CallFunc_BreakVector2D_X' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_NormCoords, CallFunc_BreakVector2D_Y) == 0x00002C, "Member 'Umg_coords_C_NormCoords::CallFunc_BreakVector2D_Y' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_NormCoords, CallFunc_Lerp_ReturnValue) == 0x000030, "Member 'Umg_coords_C_NormCoords::CallFunc_Lerp_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_NormCoords, CallFunc_Lerp_ReturnValue_1) == 0x000034, "Member 'Umg_coords_C_NormCoords::CallFunc_Lerp_ReturnValue_1' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_NormCoords, CallFunc_MakeVector2D_ReturnValue) == 0x000038, "Member 'Umg_coords_C_NormCoords::CallFunc_MakeVector2D_ReturnValue' has a wrong offset!");

// Function umg_coords.umg_coords_C.spawnDirs
// 0x0058 (0x0058 - 0x0000)
struct Umg_coords_C_SpawnDirs final
{
public:
	int32                                         Temp_int_Array_Index_Variable;                     // 0x0000(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	int32                                         Temp_int_Loop_Counter_Variable;                    // 0x0004(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	int32                                         CallFunc_Add_IntInt_ReturnValue;                   // 0x0008(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	uint8                                         Pad_C[0x4];                                        // 0x000C(0x0004)(Fixing Size After Last Property [ Dumper-7 ])
	class UUmg_coordArrow_C*                      CallFunc_Create_ReturnValue;                       // 0x0010(0x0008)(ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	class UCanvasPanelSlot*                       CallFunc_AddChildToCanvas_ReturnValue;             // 0x0018(0x0008)(ZeroConstructor, InstancedReference, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	struct FStruct_signal_spawn                   CallFunc_Array_Get_Item;                           // 0x0020(0x002C)(IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	int32                                         CallFunc_Array_Length_ReturnValue;                 // 0x004C(0x0004)(ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
	bool                                          CallFunc_Less_IntInt_ReturnValue;                  // 0x0050(0x0001)(ZeroConstructor, IsPlainOldData, NoDestructor)
};
static_assert(alignof(Umg_coords_C_SpawnDirs) == 0x000008, "Wrong alignment on Umg_coords_C_SpawnDirs");
static_assert(sizeof(Umg_coords_C_SpawnDirs) == 0x000058, "Wrong size on Umg_coords_C_SpawnDirs");
static_assert(offsetof(Umg_coords_C_SpawnDirs, Temp_int_Array_Index_Variable) == 0x000000, "Member 'Umg_coords_C_SpawnDirs::Temp_int_Array_Index_Variable' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_SpawnDirs, Temp_int_Loop_Counter_Variable) == 0x000004, "Member 'Umg_coords_C_SpawnDirs::Temp_int_Loop_Counter_Variable' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_SpawnDirs, CallFunc_Add_IntInt_ReturnValue) == 0x000008, "Member 'Umg_coords_C_SpawnDirs::CallFunc_Add_IntInt_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_SpawnDirs, CallFunc_Create_ReturnValue) == 0x000010, "Member 'Umg_coords_C_SpawnDirs::CallFunc_Create_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_SpawnDirs, CallFunc_AddChildToCanvas_ReturnValue) == 0x000018, "Member 'Umg_coords_C_SpawnDirs::CallFunc_AddChildToCanvas_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_SpawnDirs, CallFunc_Array_Get_Item) == 0x000020, "Member 'Umg_coords_C_SpawnDirs::CallFunc_Array_Get_Item' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_SpawnDirs, CallFunc_Array_Length_ReturnValue) == 0x00004C, "Member 'Umg_coords_C_SpawnDirs::CallFunc_Array_Length_ReturnValue' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_SpawnDirs, CallFunc_Less_IntInt_ReturnValue) == 0x000050, "Member 'Umg_coords_C_SpawnDirs::CallFunc_Less_IntInt_ReturnValue' has a wrong offset!");

// Function umg_coords.umg_coords_C.initTutor
// 0x0008 (0x0008 - 0x0000)
struct Umg_coords_C_InitTutor final
{
public:
	class APanel_coords_tutor2_C*                 Panel_0;                                           // 0x0000(0x0008)(BlueprintVisible, BlueprintReadOnly, Parm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
};
static_assert(alignof(Umg_coords_C_InitTutor) == 0x000008, "Wrong alignment on Umg_coords_C_InitTutor");
static_assert(sizeof(Umg_coords_C_InitTutor) == 0x000008, "Wrong size on Umg_coords_C_InitTutor");
static_assert(offsetof(Umg_coords_C_InitTutor, Panel_0) == 0x000000, "Member 'Umg_coords_C_InitTutor::Panel_0' has a wrong offset!");

// Function umg_coords.umg_coords_C.dreamInv
// 0x0018 (0x0018 - 0x0000)
struct Umg_coords_C_DreamInv final
{
public:
	TArray<struct FStruct_save>                   Invv;                                              // 0x0000(0x0010)(BlueprintVisible, BlueprintReadOnly, Parm, OutParm, ReferenceParm)
	class ADreamBase_C*                           Base;                                              // 0x0010(0x0008)(Parm, OutParm, ZeroConstructor, IsPlainOldData, NoDestructor, HasGetValueTypeHash)
};
static_assert(alignof(Umg_coords_C_DreamInv) == 0x000008, "Wrong alignment on Umg_coords_C_DreamInv");
static_assert(sizeof(Umg_coords_C_DreamInv) == 0x000018, "Wrong size on Umg_coords_C_DreamInv");
static_assert(offsetof(Umg_coords_C_DreamInv, Invv) == 0x000000, "Member 'Umg_coords_C_DreamInv::Invv' has a wrong offset!");
static_assert(offsetof(Umg_coords_C_DreamInv, Base) == 0x000010, "Member 'Umg_coords_C_DreamInv::Base' has a wrong offset!");

}

